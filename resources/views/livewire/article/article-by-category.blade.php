<div>
    ...
    @foreach ($articles as $article)
        <div
            ...
            <div class="w-full lg:w-[40%] px-5 text-zinc-500 dark:text-zinc-400 line-clamp-2 hidden lg:block">
                @php
                    $text = strip_tags($article->body); // Hapus tag HTML
                    $words = explode(' ', $text); // Pisahkan teks menjadi array kata
                    $limitedWords = array_slice($words, 0, 100); // Ambil 100 kata pertama
                    $limitedText = implode(' ', $limitedWords); // Gabungkan kembali menjadi string
                @endphp
